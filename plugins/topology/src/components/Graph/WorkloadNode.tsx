...
const InnerWorkloadNode = observer(({ element, onSelect, ...rest }: InnerWorkloadNodeProps) => {
  ...
  const urlDecorator = React.useMemo(() => {
    ...
    return (
      <UrlDecorator url={workloadData.url} radius={decoratorRadius} x={x + offset} y={y - offset} />
    );
  ...
  return (
    <g className="bs-topology-workload-node">
      <BaseNode
        ...
        element={element}
        nodeStatus={!showDetails ? getAggregateStatus(donutStatus) : undefined}
