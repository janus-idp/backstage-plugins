...
export function readThreeScaleApiEntityConfigs(config: Config): ThreeScaleConfig[] {
  const providerConfigs = config.getOptionalConfig('catalog.providers.threeScaleApiEntity');
  if (!providerConfigs) {
    return [];
  }
  return providerConfigs
    .keys()
    .map((id) => readThreeScaleApiEntityConfig(id, providerConfigs.getConfig(id)));
}
