...
export const proxyStatusMessage = (
  ...
  if (syncedProxies < desiredReplicas) {
    ...
    return ` (${unsynced}${unsynced !== 1 ? ' proxies' : ' proxy'} unsynced)`;
  }
  return '';
};
...
export class WorkloadHealth extends Health {
    ...
    if (workloadStatus) {
      ...
      if (podsStatus !== NA && podsStatus !== HEALTHY) {
        item.children = [
          ...
          {
            status: podsStatus,
            text: `${workloadStatus.availableReplicas} available pod${
        ...
        if (workloadStatus.syncedProxies >= 0) {
          item.children.push({
            status: podsStatus,
            text: `${workloadStatus.syncedProxies} synced prox${
  ...
  constructor(
    ...
    super(
      WorkloadHealth.computeItems(ns, workload, workloadStatus, requests, ctx),
    );
  }
}
