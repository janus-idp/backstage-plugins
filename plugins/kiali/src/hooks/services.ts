...
const useServiceDetail = (
  ...
  React.useEffect(() => {
    ...
    let getDetailPromise = kialiClient.getServiceDetail(namespace, serviceName, false, cluster, duration);
    let getGwPromise = kialiClient.getAllIstioConfigs([], ['gateways'], false, '', '', cluster);
    let getPeerAuthsPromise = kialiClient.getIstioConfig(namespace, ['peerauthentications'], false, '', '', cluster);
    ...
    allPromise.promise
      .then(results => {
        ...
        setPeerAuthentications(results[2].peerAuthentications);
        setFetchError(null);
        setIsLoading(false);
      })
      .catch(error => {
...
export const useServiceDetailForGraphNode = (
  ...
  React.useEffect(() => {
    if (fetchError !== null) {
      kialiState.alertUtils!.addError('Could not fetch Service Details.', fetchError);
    }
  }, [fetchError]);
