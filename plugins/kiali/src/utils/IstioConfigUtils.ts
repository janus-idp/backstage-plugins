...
export const isValidConnectionPool = (
  connectionPool: ConnectionPoolSettings,
): boolean => {
  if (connectionPool.tcp) {
    if (
      connectionPool.tcp.connectTimeout &&
      !isValidDuration(connectionPool.tcp.connectTimeout)
    ) {
      return false;
    }
    if (connectionPool.tcp.tcpKeepalive) {
      if (
        connectionPool.tcp.tcpKeepalive.interval &&
        !isValidDuration(connectionPool.tcp.tcpKeepalive.interval)
      ) {
        return false;
      }
      if (
        connectionPool.tcp.tcpKeepalive.time &&
        !isValidDuration(connectionPool.tcp.tcpKeepalive.time)
      ) {
        return false;
      }
    }
  ...
  if (connectionPool.http) {
    if (
      ...
      !isValidDuration(connectionPool.http.idleTimeout)
    ) {
      return false;
    }
  }
