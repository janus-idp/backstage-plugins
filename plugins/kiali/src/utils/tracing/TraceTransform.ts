...
class TreeNode {
  ...
  static iterFunction(
    fn: (arg0: any, arg1: any, arg2: number) => any,
    depth = 0,
  ) {
    return (node: { value: any }) => fn(node.value, node, depth);
  }
  ...
  static searchFunction(search: any) {
    ...
    return (value: any, node: TreeNode) =>
      search instanceof TreeNode ? node === search : value === search;
  }

  constructor(value: string, children = []) {
    this.value = value;
  ...
  walk(
    fn: {
      (): number;
      (spanID: string, node: TreeNode, depth?: number): void;
...
function deduplicateTags(spanTags: Array<KeyValuePair>) {
  ...
  const tags: Array<KeyValuePair> = spanTags.reduce<Array<KeyValuePair>>(
    (uniqueTags, tag) => {
...
export function getTraceSpanIdsAsTree(trace: TraceData<SpanData>) {
  ...
  const comparator = (a: { value: string }, b: { value: string }) =>
    spansSort(spansById.get(a.value)!, spansById.get(b.value)!);
  trace.spans.forEach(span => {
...
export const transformSpanData = (
  span: Span,
  cluster?: string,
): RichSpanData => {
 ...
 * generally requires.
 */
export function transformTraceData(
  data: TraceData<SpanData>,
  cluster?: string,
): JaegerTrace | null {
  let { traceID } = data;
  if (!traceID) {
