...
const adjustWorkloadNameFromReplicaset = (replicaset: string): string => {
  ...
  if (templateHashRegex.test(templateHashCandidate)) {
    return replicaset.substring(
      0,
      replicaset.length - templateHashCandidate.length - 1,
    );
  }
  return replicaset;
};

const extractWorkloadFromPod = (
  pod: string,
  ns: string,
): WorkloadAndNamespace | undefined => {
...
export const getWorkloadFromSpan = (
  span: Span,
): WorkloadAndNamespace | undefined => {
  ...
  if (nodeKV) {
    ...
    if (nsIdx >= 0) {
      return extractWorkloadFromPod(
        podWithNamespace.substring(0, nsIdx),
        podWithNamespace.substring(nsIdx + 1),
      );
    }
    return undefined;
  }
...
export const findParent = (span: Span): Span | undefined => {
  if (Array.isArray(span.references)) {
    const ref = span.references.find(
      s => s.refType === 'CHILD_OF' || s.refType === 'FOLLOWS_FROM',
    );
...
export const findChildren = (span: Span, trace: JaegerTrace): Span[] => {
  return trace.spans
    .filter(s => findParent(s)?.spanID === span.spanID)
    .sort(spansSort);
};

export const searchParentWorkload = (
  span: Span,
): WorkloadAndNamespace | undefined => {
