...
import {
  kioskDurationAction,
  kioskTimeRangeAction,
} from '../Kiosk/KioskActions';
...
export function TimeDurationModal(props: Props) {
  ...
  const reduxRefreshInterval = useKialiSelector(
    state => state.userSettings.refreshInterval,
  );
  const reduxTimeRange = useKialiSelector(
    state => state.userSettings.timeRange,
  );

  const [duration, setDuration] = React.useState(reduxDuration);
  const [refreshInterval, setRefreshInterval] =
    React.useState(reduxRefreshInterval);
  ...
  function handleCancel() {
    ...
    HistoryManager.setParam(
      URLParam.REFRESH_INTERVAL,
      String(reduxRefreshInterval),
    );
