...
export class ChartWithLegend<
  T extends RichDataPoint,
  O extends LineInfo,
> extends React.Component<Props<T, O>, State> {
  ...
  private normalizeOverlay = (factor: number): (VCDataPoint & O)[] => {
    ...
    return this.props.overlay!.vcLine.datapoints.map(dp => ({
      ...dp,
      y: dp.y * factor,
      scaleFactor: factor,
    }));
  ...
  private buildFilteredLegendData = (
    fullLegendData: LegendItem[],
  ): LegendItem[] => {
  ...
  private buildFullLegendData = (): LegendItem[] => {
    return this.props.data.map(s => {
      ...
      if (this.state.hiddenSeries.has(s.legendItem.name)) {
        return {
          name,
          symbol: { ...s.legendItem.symbol, fill: PFColors.Color200 },
        };
      }
        ...
        {this.props.data
          .map((serie, idx) => {
            ...
            if (this.props.showTrendline === true && serie.datapoints[0]) {
              const first_dpx =
                (serie.datapoints[0].x as Date).getTime() / 1000;
  ...
  private renderCategories = (): React.ReactNode => {
    ...
    const size =
      ((this.props.sizeRatio ?? 1) * this.state.width) / Math.max(nbSeries, 1);
  ...
  render(): React.ReactNode {
    ...
    const mainMax = Math.max(
      ...this.props.data.map(line =>
        Math.max(...line.datapoints.map(d => d.y)),
      ),
    );
    ...
    if (this.props.overlay) {
      ...
      const overlayMax = Math.max(
        ...this.props.overlay.vcLine.datapoints.map(d => d.y),
      );
      ...
      if (
        this.props.unit === this.props.overlay.info.lineInfo.unit &&
        overlayFactor > 0.5 &&
        overlayFactor < 2
      ) {
      ...
      if (this.props.overlay.info.buckets) {
        ...
        const model: O = {
          ...this.props.overlay.info.lineInfo,
    ...
    const labelComponent = this.props.labelComponent ? (
      ...
      <CustomTooltip showTime {...tooltipHooks} />
    );

    const filteredData = this.props.data.filter(
      s => !this.state.hiddenSeries.has(s.legendItem.name),
    );

    const voronoiProps = getVoronoiContainerProps(
      labelComponent,
      () => this.mouseOnLegend,
    );
    ...
    const chart = (
      <div
        ...
        style={{ marginTop: 0, height: chartHeight }}
      >
        <Chart
          ...
          >
            {fullLegendData.map((ld: LegendItem, idx: number) => (
              <div key={`full_legend_${idx}`} className={fullLegendStyle}>
                <div style={{ backgroundColor: ld.symbol.fill }} />
                {ld.name}
              </div>
            ))}
          </div>
