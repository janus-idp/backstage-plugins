...
test.describe('Bulk import plugin', () => {
  ...
  test('Repositories tab is shown', async () => {
    await expect(page.getByText('Added repositories (6)')).toBeVisible();
    const columns = [
      'Name',
      'Repo URL',
      'Organization',
  ...
  test('Add repositories page is shown', async () => {
    ...
    await page.click('input[aria-label="select all repositories"]');
    await expect(
      page.getByRole('heading', { name: 'Selected repositories (10)' }),
    ).toBeVisible({
      timeout: 20000,
    });
  ...
  test('Select Repositories side panel is shown', async () => {
    ...
    await page.locator(`button`).filter({ hasText: 'Organization' }).click();
    await expect(
      page.locator('tr:has-text("org/pet-store-boston") >> text=2/10'),
    ).toBeVisible();
    await page
  ...
  test('Side panel selected repositories are shown in table', async () => {
    ...
    await page.getByPlaceholder('Search in organization').fill('pet-app');
    ...
    await expect(
      page.locator('tr:has-text("org/pet-store-boston") >> text=1'),
    ).toBeVisible();
    await expect(
