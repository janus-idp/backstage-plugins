...
import type {GithubCredentials, GithubIntegrationConfig} from "@backstage/integration";
...
export async function getAllAppOrgs(
    ...
    const resp =
        await githubCredentialsProvider.getAllAppInstallations(ghConfig);
...
export async function addGithubAppOrgs(
    ...
    let totalCount = 0;
    try {
        const resp = await getAllAppOrgs(deps.githubCredentialsProvider, ghConfig, credentialAccountLogin);
